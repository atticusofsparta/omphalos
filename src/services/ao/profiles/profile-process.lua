...
Handlers.add('Run-Action', Handlers.utils.hasMatchingTag('Action', 'Run-Action'),
    ...
    Handlers.add('Set-Profile-Registry', Handlers.utils.hasMatchingTag('Action', 'Set-Profile-Registry'), function (msg)

        local id = msg.Tags['Profile-Registry-Id']
        if not id then
            ao.send({
                Target = msg.From,
                Action = 'Input-Error',
                Tags = {
                    Status = 'Error',
                    Message = 'Invalid arguments, required { Profile-Registry-Id }'
                }
            })
            return
        end
        ...
        ao.send({
            Target = msg.From,
            Action = 'Set-Profile-Registry-Success',
            Tags = {
                Status = 'Success',
                Message = 'Profile Registry Id set'
            }
        })
    end)